...
class BeeHive(object):
    ...
    def compute_probability(self):
        ...
        values = [bee.fitness - (max_delta * ((1 + bee.counter) / (self.max_trials))) for bee in self.population]
    ...
    def send_employee(self, index, type):
        ...
        if index:
            ...
            temp, bestBee = self._mutate(index, bee_ix, dim=d, type=type)
            ...
            if (zombee.fitness > bestBee.fitness):
                self.population[index] = zombee
                self.population[index].counter = 0

            else:
                bestBee.counter += 1
                self.population[index] = copy.deepcopy(bestBee)
    ...
    def _mutate(self, current_bee, other_bee, dim, type):
        ...
        if type == 1:
            neighboors = []
            if self.neighboor_d<sys.maxsize:
        ...
        return (self.population[other_bee].vector[dim] + \
               (random.random() - 0.5) * 2 * \
               (best_Bee.vector[dim] - self.population[other_bee].vector[dim]), best_Bee)
