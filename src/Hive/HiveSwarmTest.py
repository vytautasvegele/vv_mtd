...
class BeeHive(object):
    ...
    def __init__(self,
        ...
        """
        print("-------------------")
        print("Starting iqsABC algorithm")
        print("Finding patch for starting area of:", start_x, " ",  start_y)
        print("-----------------------------")
        # checks input
        ...
        self.r = r
        self.population = [Bee(lower, upper, fun, start_x=self.start_x, start_y=self.start_y, r=self.r) for i in range(self.size)]
    ...
    def send_scout(self):
        ...
        if (trials[index] > newLimit):
            # creates a new scout bee randomly
            self.fitcount += 1
            self.population[index] = Bee(self.lower, self.upper, self.evaluate, r=self.r)
